...
function getPromise(number) {
  return new Promise((resolve, reject) => {
    if (number >= 0) resolve('promise that will be fulfilled');
    else reject(new Error('promise that will be rejected'));
  });
}
...
function getPromiseResult(source) {
  return new Promise((resolve) => {
    const res = source.then(() => 'success').catch(() => 'fail');
    resolve(res);
  });
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((result) => {
    const arr = [];
    result.forEach((el) => {
      if (el.status === 'fulfilled') arr.push(el.value);
      else arr.push(null);
    });
    return arr;
  });
...
function queuePromises(promises) {
  return promises.reduce((acc, curr) => {
    return acc.then((res) => {
      return curr.then((single) => res + single);
    });
  }, Promise.resolve(''));
}
